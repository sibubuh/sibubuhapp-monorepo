...
export async function getPage({ locale, slug }: { locale: string; slug: string }): Promise<Page> {
...
export async function getHeader({ locale }: { locale: string | null }): Promise<Navbar | null> {
...
export async function getFooter({ locale }: { locale: string | null }): Promise<Footer | null> {
  return await getDataWithLocalization({
    endpoint: '/api/footer',
    populate: [
      'localizations',
...
export async function getNewsYears(): Promise<number[]> {
  ...
  const years = [...new Set(result.data.map((post) => new Date(post.date).getFullYear()))]
...
export async function getDataWithLocalization({
  ...
  if (isValid) {
    ...
    const queryParsed = QueryString.stringify(defaultLocaleQuery, { skipNulls: true })
...
function _getDataFromRequest({
  ...
  const getSingleDataInArray = () => {
    if (Array.isArray(originalData.data) && originalData.data.length > 0) {
      return originalData.data.find((item) => item !== null && item !== undefined)
    }
    return null
  }
